(ns com.fingerhutpress.clj-vectors.bifurcan-list-vector4
...
(defn- java-hashcode [^List4 v]
  (.hashCode v))

(defn- java-tostring [^List4 v]
  (.toString v))

(defn- get-nth [^List4 v idx]
  (.nth v idx))

(defn- append-one-elem [^List4 v elem]
  (.addLast v elem))

(defn- remove-last-elem [^List4 v]
  (.removeLast v))

(defn- replace-elem [^List4 v idx new-elem]
  (let [i (long idx)]
    (.set v i new-elem)))
...
(defn- subvec* [^List4 v start end size]
  (let [s (long start)
        e (long end)]
    (if (<= 0 s e size)
      (if (== s e)
...
(defn- splicev [^BifurcanListVector v1 ^BifurcanListVector v2]
  ...
  (when-not (instance? List4 (.v v1))
    (println "----------------------------------------------------------------------")
    (println "(class (.v v1))=" (class (.v v1)))
    (flush)
    (assert (instance? List4 (.v v1))))
  (when-not (instance? List4 (.v v2))
    (println "----------------------------------------------------------------------")
    (println "(class (.v v2))=" (class (.v v2)))
    (flush)
    (assert (instance? List4 (.v v2))))
  (let [^List4 bv1 (.v v1)
        ^List4 bv2 (.v v2)
        c (.concat bv1 bv2)]
    (assert (instance? List4 c))
...
(defn check-invariants
  ...
  (let [^List4 v (.v vec)]
    ...
    ;; about the List4 data structure v.
    {:error false}))

(defn height [^BifurcanListVector vec]
  (let [^List4 v (.v vec)
        ^ListNodes4$Node root (.get list-root-field v)]
    (if (nil? root)
      1
      (let [s (.shift root)]
...
(defn max-supported-height []
  ...
  ;; List4 tree, but not sure.  I will try that out and see if any
...
;; elems?  Maybe because bifurcan List4 calls Java .hashCode on each
